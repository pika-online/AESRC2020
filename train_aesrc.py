...
def train():
    ...
    parallel_model = mdl.compile(model,gpus=3,lr=0.0001,
...
if __name__ == "__main__":
    ...
    ACCENT_CLASSES = 8
    EPOCHS = 30
    RAW_MODEL = '/disc1/ARNet/exp/aesrc/003.h5'
    LOG_FILE = 'exp/aesrc/model.csv'
    MODEL_DIR = "exp/aesrc"
    ...
    lr_reducer = ReduceLROnPlateau(factor=0.5, cooldown=0, patience=1, min_lr=0.5e-6,
                                   monitor='accent_labels_loss',min_delta=0.1,verbose=1)
    early_stopper = EarlyStopping( min_delta=0.01, patience=3,monitor='accent_labels_loss',verbose=1)
