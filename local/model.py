...
from keras.layers import Input, Dense, Lambda,Dropout, Reshape,LSTM,Conv2D,MaxPooling2D,\
    Bidirectional,GRU,GlobalAveragePooling1D,Layer,AveragePooling1D,BatchNormalization
...
def model_res_gru_ctc(shapes,
    ...
    if cnn == "res18":cnn1 = resnet18_(shapes)
    if cnn == "res34":cnn1 = resnet34_(shapes)
    if cnn == "res50":cnn1 = resnet50_(shapes)
    if cnn == "res101":cnn1 = resnet101_(shapes)
    if cnn == "res152":cnn1 = resnet152_(shapes)
    ...
    model = Model(inputs=[inputs, ctc_labels, ctc_input_len, ctc_label_len],
                  outputs=[ctc_loss], name="model_res_gru_ctc")
    model.summary()
    if raw_model:
...
def model_res_gru_ctc_accent(shapes,
                             bpe_classes,
                             accent_classes,
                             max_label_len,
                             cnn='res18',
...
if __name__=="__main__":
    ...
    model = model_res_gru_ctc(shapes=[1200, 80, 1],
                              bpe_classes=1000,
                              max_label_len=72)
