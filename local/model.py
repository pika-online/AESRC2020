...
from keras.layers import Input, Dense, Lambda,Dropout, Reshape,LSTM,\
    Bidirectional,GRU,GlobalAveragePooling1D,Layer,AveragePooling1D
...
import local.transformer as tfr
...
"""
====================================
        Attention-CTC Models
====================================
"""

def self_att(x):
    return SeqSelfAttention(
        units=128,
        attention_width=20,
...
def model_att_ctc(shapes,
    ...
    encoder = cnn2(encoder)
    encoder = cnn3(encoder)
    encoder = self_att(encoder)
    encoder = Dropout(0.3)(encoder)

    # CTC
    ctc = Dense(256, activation="relu", kernel_initializer='he_normal', name='ctc')(encoder)
    ctc = Dropout(0.3)(ctc)
    ctc_pred = Dense(bpe_classes, activation="softmax", kernel_initializer='he_normal', name='ctc_pred')(ctc)
    ctc_input_len = Input(shape=[1], dtype='int32', name='ctc_input_len')
    ctc_label_len = Input(shape=[1], dtype='int32', name='ctc_label_len')
...
def model_tfr_ctc(shapes,
    ...
    encoder = cnn2(encoder)
    encoder = cnn3(encoder)
    encoder = tfr.EncoderLayer(d_model=256,d_inner_hid=1024,n_head=8)(encoder)
    encoder = Dropout(0.3)(encoder)
    ...
    # MODEL
    model = Model(inputs=[inputs, ctc_labels, ctc_input_len, ctc_label_len],
                  outputs=[ctc_loss], name="model_tfr_ctc")
    model.summary()
    if raw_model:
        print("===== init weights from:%s =====" % raw_model)
        model.load_weights(raw_model, by_name=True, skip_mismatch=True)
...
def model_att_ctc_accent(shapes,
                     bpe_classes,
                     accent_classes,
                     max_label_len,
                     raw_model=None
    ...
    # ACCENT
    accents = GlobalAveragePooling1D()(encoder)
    accents = Dense(256, activation="relu", kernel_initializer='he_normal')(accents)
    accents = Dropout(0.3)(accents)
    aesrc_nation = Dense(accent_classes, activation='softmax', kernel_initializer='he_normal',name="accent_labels")(accents)
    ...
    # MODEL
    model = Model(inputs=[inputs, ctc_labels, ctc_input_len, ctc_label_len],
                  outputs=[ctc_loss, aesrc_nation ], name="model_tfr_ctc_accents")
    model.summary()
    if raw_model:
        print("===== init weights from:%s =====" % raw_model)
        model.load_weights(raw_model, by_name=True,skip_mismatch=True)
...
def model_tfr_ctc_accent(shapes,
                     bpe_classes,
                     accent_classes,
                     max_label_len,
                     raw_model=None
...
if __name__=="__main__":
    ...
    model = model_tfr_ctc(shapes=[1200,80,1],
                             bpe_classes=1000,
                             max_label_len=72)
    # model = model_tfr_ctc_accent(shapes=[1200, 80, 1],
    #                             bpe_classes=1000,
    #                             max_label_len=72,
    #                              accent_classes=8)
